document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM ELEMENTS (THIS SECTION IS NOW COMPLETE AND CORRECT) ---
    ...
    async function init() {
        try {
            ...
            const numericField = Object.keys(PDSL.manifest.index_shard_map).find(k => k.includes('count')) || 'profile.member_count';
            const numericRepoNum = PDSL.manifest.index_shard_map[numericField] || 1;
            const numericManifestUrl = bustCache(`${getRepoUrl(numericRepoNum)}/numeric_manifest.json`);
    ...
    async function fetchAndRenderBatch(shouldRender = true) { if (PDSL.isFetching || PDSL.currentIndex >= PDSL.allServerIds.length) return; PDSL.isFetching = true; const batchSize = shouldRender ? 20 : 500; const batchIds = PDSL.allServerIds.slice(PDSL.currentIndex, PDSL.currentIndex + batchSize); PDSL.currentIndex += batchSize; const fetchesByUrl = new Map(); batchIds.forEach(id => { const url = getUrlForDataFile(id); if (!fetchesByUrl.has(url)) fetchesByUrl.set(url, []); }); const dataPromises = Array.from(fetchesByUrl.keys()).map(url => fetchJSON(url)); const dataChunks = await Promise.all(dataPromises); const serverMap = new Map(dataChunks.flat().filter(Boolean).map(item => [item.internal_id, item])); batchIds.forEach(id => { const serverItem = serverMap.get(id); if (serverItem && applyPostFilters(serverItem)) { PDSL.loadedServers.push(serverItem); } }); if (shouldRender) updateDisplay(); PDSL.isFetching = false; }
    async function performSearch(isInitialLoad = false) { if (!PDSL.manifest) { statusText.textContent = "Search system not loaded. Please wait."; return; } const query = searchInput.value.trim(); if (query === "" && !isInitialLoad) return; resetSearchState(); statusText.textContent = 'Loading...'; try { if (query === "") { statusText.textContent = 'Loading most popular servers...'; const sortedListUrl = bustCache(`${getRepoUrl(1)}/all_servers_sorted_by_members.json`); const sortedIds = await fetchJSON(sortedListUrl); PDSL.allServerIds = sortedIds || []; } else { statusText.textContent = 'Executing query plan...'; const plan = createQueryPlan(query); PDSL.postFilters = plan.postFilters; const finalServerIds = await executeQueryPlan(plan); if (finalServerIds.length === 0) { statusText.textContent = 'No results found.'; return; } statusText.textContent = `Found ${finalServerIds.length} potential servers. Loading...`; PDSL.allServerIds = finalServerIds; } await fetchAndRenderBatch(); } catch (error) { console.error("Query failed:", error); statusText.textContent = `Error: ${error.message}`; } }
    function resetSearchState() { PDSL.isFetching = false; PDSL.allServerIds = []; PDSL.currentIndex = 0; PDSL.postFilters = []; PDSL.loadedServers = []; resultsTableBody.innerHTML = ''; statusText.textContent = ''; }
    ...
    window.addEventListener('scroll', () => { if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 500) { if (!PDSL.isFetching) fetchAndRenderBatch(); } });
    init();
});